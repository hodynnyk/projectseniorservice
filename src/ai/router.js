...
- R2/voice наразі вимкнені: не обіцяй збереження великих файлів або голосу, якщо модуль вимкнений.
...
export async function getAiRouterConfig(env) {
  ...
  return {
    ...
    providers: {
      openai: { configured: !!openaiKey, model: openaiModel, role: active === 'openai' ? 'active' : fallback === 'openai' ? 'fallback' : 'available' },
...
export async function askAI(env, user, { instructions = '', input = '', context = [], source = 'bot', preferredProvider = '' } = {}) {
  ...
  for (const provider of ordered) {
    ...
    attempts.push({ provider, ok: !!res.ok, status: res.status || null, ms: Date.now() - started, error: res.error || (!res.ok ? res.text : '') });
    if (res.ok && res.text) {
...
function buildRouterFailureText(user, attempts) {
  const details = attempts.map(a => `${a.provider}: ${a.skipped ? 'немає ключа' : a.status || a.error || 'error'}`).join('; ');
