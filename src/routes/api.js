...
export async function handleApi(env, request, ctx) {
  ...
  if (path === '/api/health') {
    ...
    return json({ ok: true, service: 'projectseniorservice', version: 'sonya-v18-clean-miniapp-secure-admin', time: new Date().toISOString(), configured: !!setup?.configured, bindings: { kv: !!env.SONYA_KV, d1: !!env.DB, files: 'metadata_only', google: true, gemini: true } });
  }
